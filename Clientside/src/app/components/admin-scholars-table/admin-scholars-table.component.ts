...
export class AdminScholarsTableComponent {
  ...
  fetchDataWithFilters(): void {
    const filters = this.scholarCategory.values; // Get all dropdown selected values
    this.url =
      this.http.serverUrl +
      `scholars/${this.scholarCategory}/display-scholars-by-category`; // Assuming 'filter-scholars' is your filter endpoint

    this.loading = true;
    this.http.getDataF(this.url, filters).subscribe({
      next: (response) => {
        this.notificationApi.alertSuccess('Filter Applied!');
  ...
  onFilterChange(): void {
    this.fetchDataWithFilters(); // Call function to fetch filtered data
  }

  // downloadTable(): void {
  //   const csvContent = this.convertTableToCSV(); // Convert table data to CSV format
